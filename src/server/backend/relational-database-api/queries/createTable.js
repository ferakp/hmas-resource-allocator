...
export const ALGORITHMS =
  "CREATE TABLE IF NOT EXISTS algorithms ( id SERIAL PRIMARY KEY, type VARCHAR NOT NULL, name VARCHAR NOT NULL, created_on timestamp NOT NULL, updated_on timestamp NOT NULL, created_by BIGINT NOT NULL REFERENCES users (id) ON DELETE CASCADE)";

export const TASKS =
  "CREATE TABLE IF NOT EXISTS tasks ( id SERIAL PRIMARY KEY, type VARCHAR NOT NULL, name VARCHAR NOT NULL, description VARCHAR, estimated_time DECIMAL, knowledge_tags VARCHAR, resource_demand VARCHAR, priority BIGINT DEFAULT 0, created_on timestamp NOT NULL, created_by bigint NOT NULL REFERENCES users (id) ON DELETE CASCADE, start_date timestamp, due_date timestamp, assigned_to VARCHAR, updated_on timestamp NOT NULL)";

export const DASHBOARD_SETTINGS =
  "CREATE TABLE IF NOT EXISTS dashboard_settings ( id BIGINT PRIMARY KEY REFERENCES users (id) ON DELETE CASCADE, settings VARCHAR NOT NULL, created_on timestamp NOT NULL, updated_on timestamp NOT NULL )";

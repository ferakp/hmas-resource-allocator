...
export const HOLONS =
  "CREATE TABLE IF NOT EXISTS holons ( id SERIAL PRIMARY KEY, type VARCHAR NOT NULL, name VARCHAR NOT NULL, gender VARCHAR, daily_work_hours DECIMAL NOT NULL, latest_state VARCHAR, remote_address VARCHAR, api_token VARCHAR, availability_data VARCHAR NOT NULL, load_data VARCHAR NOT NULL, stress_data VARCHAR NOT NULL, cost_data VARCHAR NOT NULL, age DECIMAL, experience_years DECIMAL DEFAULT 0, is_available BOOLEAN NOT NULL DEFAULT TRUE, created_on timestamp NOT NULL, created_by BIGINT REFERENCES users (id) ON DELETE CASCADE, updated_on timestamp NOT NULL )";

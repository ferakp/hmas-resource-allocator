/**
 * FIND BY SPECIALIZATION ALGORITHM
 *
  ...
  try {
    ...
    if (Array.isArray(holon.graphRecords)) {
      holon.graphRecords.forEach((record) => {
        ...
        if (relations.length === 1 && relations[0].type.toLowerCase() === 'specialized_task_type' && target.labels[0].toLowerCase() === 'type') {
          if (target.properties.name) specializationTypes.push(target.properties.name.toLowerCase());
        }
      });
    }
    ...
    if (specializationTypes.length > 0) {
      let tempScore = 0;
      specializationTypes.forEach((i) => {
        if (i === task.type.toLowerCase()) tempScore += 2;
      });

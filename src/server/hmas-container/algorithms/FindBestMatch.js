/**
 * FIND BEST MATCH ALGORITHMS
 *
 * Runs algorithm with given parameters
 * ALGORITHMS PHASES
 * PHASE 1 - Sort tasks according their priorities and choose only tasks whose start_date has started and that are not completed
...
export const run = (tasks, holons) => {
  try {
    ...
    const availableHolons = holons.filter((i) => {
      if (i.is_available === true && i.availability_data.currentValue >= 0 && i.availability_data.currentValue < 1 && i.daily_work_hours > 0) return true;
      else return false;
    });
    // PHASE 3
    ...
    // other values means unexpected error occured
      ...
      return JSON.stringify({ error: "HMAS couldn't run algorithm due to an unexpected error" });
    }
  }
};

/**
 * Sort tasks according to their priorities
 * Tasks with higher priorities are moved to the end of line
 * @param {array} tasks tasks to be sorted
 */
export const sortTasksPriority = (tasks) => {
  tasks.sort((a, b) => {
    let response = 0;
    if (a.priority === null && b.priority !== null) response = -1;
    else if (a.priority !== null && b.priority === null) response = 1;
    else if (a.priority === b.priority) response = 0;
    else if (a.priority < b.priority) response = -1;
    else if (a.priority > b.priority) response = 1;
    return response;
  });
};
 ...
 * @param {array} holons array of standard holons
...
export const findBestMatch = (task, holons) => {
  try {
    ...
    while (!isReady) {
      ...
      const availableHolons = holons.filter(
        (i) => i.availability_data.currentValue >= 0 && i.availability_data.currentValue < 1 && typeof i.daily_work_hours === 'number' && i.daily_work_hours > 0
      );
